...
# lets now get the name and paths of each file in each subdirectory in our training set (database)

data_base_path = os.path.join(curr_path, 'database')

filenames = []
# os.walk: list everything contains the path 'data_base_path'
for path, subdirs, files in os.walk(data_base_path):
   for name in files:
      filenames.append(os.path.join(path, name))
      #filenames.append(name)            the names of files
